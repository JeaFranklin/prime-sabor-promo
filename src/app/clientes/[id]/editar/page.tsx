...
export default function EditarCliente() {
  ...
  const [form, setForm] = useState({
    nome_empresa: '', cnpj: '', inscricao_estadual: '', categoria: '',
    ...
    async function carregar() {
      ...
      if (cData) {
        setForm({
          ...
          status: cData.status || 'ativo',
        })
        if (cData.lat && cData.lng) setCoordenadas({ lat: cData.lat, lng: cData.lng })
      }
      if (ctData && ctData.length > 0) setContatos(ctData)
      else setContatos([contatoVazio()])
      setCarregando(false)
    }
    if (id) carregar()
  }, [id])
  ...
  async function salvar() {
    ...
    // Apaga contatos antigos e reinsere
    ...
    if (contatosValidos.length > 0) {
      await supabase.from('contatos_cliente').insert(
        contatosValidos.map(({ id: _id, ...c }) => ({ ...c, cliente_id: id }))
      )
    }
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
          ...
          <div className="space-y-3">
            ...
            {coordenadas && (
              <p className="text-xs text-green-600 text-center font-semibold">
                ✅ Localização salva — promotoras conseguirão navegar até aqui.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Contatos com 💬 receberão notificação via WhatsApp quando um serviço for agendado.
          </p>
